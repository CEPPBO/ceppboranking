...
function load() {
  ...
  if (HAS_DB) {
    window.db.ref('votos').on('value', (snap) => {
      votos = snap.val() || {};
      renderProfes();
      renderRanking();
    });
...
function votar(profeId, delta = 1) {
  if (HAS_DB) {
    ...
    window.db.ref('votos/' + profeId).transaction(cur => {
...
function importarProfes(archivo) {
  ...
  reader.onload = e => {
    try {
      ...
      if (HAS_DB) {
        const updates = {};
        normalizados.forEach(p => updates[p.id] = p);
        window.db.ref('profes').update(updates, (err)=>{
          if (err) alert('Error subiendo a Firebase: '+err);
          else toast(`Importados ${normalizados.length} alumno(s).`);
        });
      } else {
        profes = normalizados;
        save(); renderProfes(); renderRanking();
        toast(`Importados ${normalizados.length} alumno(s).`);
      }
